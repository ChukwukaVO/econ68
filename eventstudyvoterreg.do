...
areg didvotereg _year* iswoman whiteshr isemployed ishisp i.year [w=vosuppwt] if targpop==1, absorb(statefip) cluster(statefip) robust
...
title("Voter Registration Event Study", size(medlarge)) ///
plotregion(lcolor(none)) legend(off) ylabel(, grid) xtitle("year")
gr export figure2reg.png, as(png) replace
...
// plotregion(lcolor(none)) legend(off) ylabel(, grid) xtitle("year")
// gr export figure2altreg.png, as(png) replace
